...
import { removeVer } from './extension';
...
export class FileExplorer {
	constructor(context: vscode.ExtensionContext, path: string, refresh?: () => void) {
		...
		vscode.commands.registerCommand('fileExplorer.deleteEntry', (resource) => {
			vscode.window
			.showWarningMessage(
				"This will remove all files of this branch or this version from your disk and have no rollback options, Please make sure all risks have been identified ?",
				...["Yes", "No"]
			)
			.then((answer) => {
				if (answer === "Yes") {
					vscode.window.showInformationMessage('remove branch', resource.uri.path);
					removeVer(resource.uri.path, () => {
						setTimeout((() => {
